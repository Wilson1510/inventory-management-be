...
class PurchaseOrder(BaseOrder):
    ...
    def _create_receipt_order_and_items(self):
        Receipt = apps.get_model('businessapp', 'Receipt')
        receipt = Receipt.objects.create(
            purchase_order=self,
            arrival_date=self.arrival_date
        )
        ReceiptItem = apps.get_model('businessapp', 'ReceiptItem')
        for item in self.items.all():
...
class SalesOrder(BaseOrder):
    ...
    def _create_delivery_order_and_items(self):
        Delivery = apps.get_model('businessapp', 'Delivery')
        delivery = Delivery.objects.create(
            sales_order=self,
            delivery_date=self.delivery_date
        )
        DeliveryItem = apps.get_model('businessapp', 'DeliveryItem')
        for item in self.items.all():

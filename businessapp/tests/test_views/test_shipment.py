...
class DeliveryViewSetTest(APITestCase):
    ...
    def test_done_bad_request(self):
        with patch('businessapp.views.Delivery.done') as mock_done:
    ...
    def test_cancel_bad_request(self):
        with patch('businessapp.views.Delivery.cancel') as mock_cancel:
...
class ReceiptViewSetTest(APITestCase):
    ...
    def test_done_bad_request(self):
        with patch('businessapp.views.Receipt.done') as mock_done:
    ...
    def test_cancel_bad_request(self):
        with patch('businessapp.views.Receipt.cancel') as mock_cancel:
            mock_cancel.side_effect = ValueError('Cannot cancel')
            self.client.force_authenticate(user=self.user_a)
            response = self.client.post(self.cancel_url)
